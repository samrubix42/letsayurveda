<div class="space-y-6 animate-fade-in-up">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="font-headline-sm text-3xl font-bold text-slate-800 tracking-tight">Product Categories</h1>
            <p class="text-sm text-slate-500 mt-1">Manage and classify your wellness sanctuary ecommerce catalog.</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 text-slate-400 font-label-caps text-[10px] tracking-widest border-b border-slate-100">
                        ...
                        <th class="py-4 px-6 font-bold">Parent Category</th>
                ...
                <tbody class="divide-y divide-slate-100 text-sm">
                    @forelse($categories as $category)
                        <tr wire:key="category-{{ $category->id }}" class="hover:bg-slate-50/40 transition-colors">
                            ...
                            <!-- Image -->
                            <td class="py-4 px-6">
                                <div class="h-10 w-10 rounded-lg overflow-hidden bg-slate-100 border border-slate-200/60 flex items-center justify-center">
                                    @if($category->image)
                            ...
                            <td class="py-4 px-6">
                                ...
                                @else
                                    <span class="text-xs text-slate-400 font-semibold italic">Main Category</span>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-12 px-6 text-center text-slate-400">
                                ...
                                <span class="text-sm font-medium">No product categories found.</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @include('admin.categorylist.form-modal')
    @include('admin.categorylist.delete-modal')
