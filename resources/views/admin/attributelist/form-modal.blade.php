<div x-data="{ modalOpen: false }"
    ...
    <div x-show="modalOpen"
        ...
        <div class="px-6 py-5 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
            <div>
                <h3 class="text-lg font-bold text-slate-800">
                    {{ $attributeId ? 'Edit Variant Attribute' : 'Create Variant Attribute' }}
                </h3>
                <p class="text-xs text-slate-500 mt-0.5">Configure details and values for your product variant options.</p>
            </div>
        ...
        <form wire:submit.prevent="save" class="p-6 space-y-5">
            ...
            <!-- Attribute Name -->
            <div class="space-y-2">
                <label class="block text-xs font-label-caps text-slate-500 uppercase font-bold tracking-wider">Attribute Name</label>
            ...
            <!-- Interactive Attribute Values -->
            <div class="space-y-2">
                <label class="block text-xs font-label-caps text-slate-500 uppercase font-bold tracking-wider">Attribute Values</label>
                
                <div class="flex gap-2">
                    <input wire:model="newValue" 
                           wire:keydown.enter.prevent="addAttributeValue"
                           type="text" 
                           class="flex-1 bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-lg px-4 py-3 outline-none text-sm transition-all focus:ring-1 focus:ring-emerald-500/20" 
                           placeholder="e.g. 100ml, Red, Mint, Ginger"/>
                    <button type="button" 
                            wire:click="addAttributeValue"
                            class="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs tracking-wider px-5 py-3 rounded-lg hover:shadow-md active:scale-95 transition-all cursor-pointer">
                        Add
                    </button>
                </div>
                @error('newValue')
                ...
                <!-- Badges of added values -->
                <div class="mt-3 p-3 bg-slate-50 rounded-xl border border-slate-200">
                    <div class="flex flex-wrap gap-2">
                        @forelse($attributeValues as $index => $val)
                            <div class="inline-flex items-center gap-1.5 bg-white border border-slate-200 rounded-full pl-3 pr-1.5 py-1 text-xs font-semibold text-slate-700">
                                <span>{{ $val['value'] }}</span>
                                <button type="button" 
                                        wire:click="removeAttributeValue({{ $index }})"
                                        class="h-4.5 w-4.5 rounded-full hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-rose-500 transition-colors">
                                    <span class="material-symbols-outlined text-xs">close</span>
                                </button>
                            </div>
                        @empty
                            <span class="text-xs text-slate-400 italic font-semibold">No values added yet. Add at least one value.</span>
                        @endforelse
                    </div>
                </div>
