<div class="space-y-6 animate-fade-in-up">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="font-headline-sm text-3xl font-bold text-slate-800 tracking-tight">Variant Attributes</h1>
            <p class="text-sm text-slate-500 mt-1">Manage and classify variant attributes (e.g. Size, Color, Flavor) and their values.</p>
        </div>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex items-center gap-2 w-full max-w-sm px-3.5 py-2.5 bg-white border border-slate-200 rounded-lg text-slate-400 focus-within:border-emerald-500 focus-within:ring-1 focus-within:ring-emerald-500/20 transition-all">
                ...
                <input wire:model.live="search" class="bg-transparent border-none text-xs outline-none text-slate-700 placeholder:text-slate-400 w-full focus:ring-0" placeholder="Filter attributes by name..." type="text"/>
            </div>
            <div class="text-xs text-slate-400 font-semibold">
                SHOWING {{ $allAttributes->count() }} OF {{ $allAttributes->total() }} RECORDS
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 text-slate-400 font-label-caps text-[10px] tracking-widest border-b border-slate-100">
                        <th class="py-4 px-6 font-bold">Attribute</th>
                        <th class="py-4 px-6 font-bold">Slug</th>
                        <th class="py-4 px-6 font-bold">Values</th>
                ...
                <tbody class="divide-y divide-slate-100 text-sm">
                    @forelse($allAttributes as $attribute)
                        <tr wire:key="attribute-{{ $attribute->id }}" class="hover:bg-slate-50/40 transition-colors">
                            
                            <!-- Attribute Name -->
                            <td class="py-4 px-6">
                                <span class="font-bold text-slate-800">{{ $attribute->name }}</span>
                            </td>

                            <!-- Slug -->
                            <td class="py-4 px-6 text-slate-500 font-mono text-xs">{{ $attribute->slug }}</td>

                            <!-- Values tags -->
                            <td class="py-4 px-6">
                                <div class="flex flex-wrap gap-1.5 max-w-md">
                                    @forelse($attribute->values as $val)
                                        <span class="inline-flex items-center px-2 py-0.5 rounded bg-slate-100 text-slate-700 border border-slate-200 text-xs font-medium">
                                            {{ $val->value }}
                                        </span>
                                    @empty
                                        <span class="text-xs text-slate-400 italic font-semibold">No values</span>
                                    @endforelse
                                </div>
                            </td>
                            ...
                            <td class="py-4 px-6 text-center">
                                <button wire:click="toggleStatus({{ $attribute->id }})" class="cursor-pointer active:scale-95 transition-transform inline-flex">
                                    @if($attribute->status)
                            ...
                            <td class="py-4 px-6 text-center text-slate-400 font-medium text-xs">
                                {{ $attribute->created_at ? $attribute->created_at->format('d M Y') : 'N/A' }}
                            </td>

                            <!-- Actions -->
                            <td class="py-4 px-6 text-right">
                                <div class="flex items-center justify-end gap-3">
                                    <button type="button" @click="$dispatch('open-modal'); $wire.openEditModal({{ $attribute->id }})" class="text-slate-500 hover:text-emerald-600 p-1.5 rounded hover:bg-slate-100 transition-all cursor-pointer" title="Edit Attribute">
                                        <span class="material-symbols-outlined text-lg">edit</span>
                                    </button>
                                    <button type="button" @click="$dispatch('open-delete-modal'); $wire.confirmDelete({{ $attribute->id }})" 
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-12 px-6 text-center text-slate-400">
                                ...
                                <span class="text-sm font-medium">No variant attributes found.</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @include('admin.attributelist.form-modal')
    @include('admin.attributelist.delete-modal')
